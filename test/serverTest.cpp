...
int main()
{
    ...
    //serveraddr.sin_addr.s_addr = htonl('100.98.139.169');
    ...
    int reuseRet = setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes));
    // bind and listen
    int bindRet = bind(listenfd, (sockaddr *)&serveraddr, sizeof(serveraddr));
    int listenRet =listen(listenfd, LISTENQ);
    printf("reuseRet:%d, bindRet:%d, listenRet:%d\n", reuseRet, bindRet, listenRet);
    int acceptNumber = 0;

    struct sockaddr_in listendAddr;
    int listendAddrLen;
    getsockname(listenfd, (struct sockaddr *)&listendAddr, (socklen_t *)&listendAddrLen);
    ...
    while (1)
    {
        ...
        acceptNumber++;
        cout << "accept index:" << acceptNumber << endl;
        //sleep(1);
    }
    ...
    for (;;)
    {
        ...
        for (i = 0; i < nfds; ++i)
        {
            cout << "events[i].events begin" << events[i].events << endl;
            ...
            cout << "events[i].events end " << events[i].events << endl;
            if (events[i].data.fd == listenfd) //如果新监测到一个SOCKET用户连接到了绑定的SOCKET端口，建立新的连接。
            {
                ...
                cout << "acceptNumber:" << acceptNumber << ", accapt a connection from " << str << endl;
                //设置用于读操作的文件描述符
